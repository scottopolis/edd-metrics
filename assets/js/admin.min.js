...
}};b.dashResponse=function(f){var h=JSON.parse(f);console.log(h);var g="% over previous "+h.dates.num_days+" days";d("#revenue").text("$"+h.earnings.total);
...
d("#renewals").text(h.renewals.count);d("#renewals-compare span").text(h.renewals.compare.percentage+g).removeClass().addClass(h.renewals.compare.classes);
d("#refunds").text(h.refunds.count);d("#refunds-compare span").text(h.refunds.compare.percentage+g).removeClass().addClass(h.refunds.compare.classes);};
...
d(".detail-compare-third").text("$"+h.earnings.detail.twelvemoago.total);break;case"renewals":break;default:}};b.getQueryVariable=function(f){var h=c.location.search.substring(1);
var j=h.split("&");for(var g=0;g<j.length;g++){var k=j[g].split("=");if(k[0]==f){return k[1];}}return(false);};jQuery(a).ready(b.init);})(window,document,jQuery);
