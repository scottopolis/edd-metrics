...
d("#revenue").html(h.earnings.total);d("#revenue-compare span").html(h.earnings.compare.percentage+g).removeClass().addClass(h.earnings.compare.classes);
d("#sales").html(h.sales.count);d("#sales-compare span").html(h.sales.compare.percentage+g).removeClass().addClass(h.sales.compare.classes);d("#yearly").html(h.earnings.avgyearly.total);
d("#avgyearly-compare span").html(h.earnings.avgyearly.compare.percentage+g).removeClass().addClass(h.earnings.avgyearly.compare.classes);d("#avgpercust").html(h.earnings.avgpercust.total);
...
var h=JSON.parse(f);var g=c.eddMetrics.compare_string+" "+h.dates.num_days+" "+c.eddMetrics.days;d("#renewals").html(h.renewals.count);d("#renewal-amount").html(h.renewals.earnings);
...
d("#refund-amount").html(h.refunds.losses);d("#refunds-compare span").html(h.refunds.compare.percentage+g).removeClass().addClass(h.refunds.compare.classes);
...
d("#discounts").html(h.discounts.now.amount);d("#discounts-count").html(h.discounts.now.count);d("#discounts-compare span").html(h.discounts.compare.percentage+g).removeClass().addClass(h.discounts.compare.classes);
};b.detailResponse=function(f){console.log("detailResponse",f);var h=JSON.parse(f);var g=b.getQueryVariable("metric");switch(g){case"revenue":d("#revenue").html(h.earnings.total);
d("#revenue-compare span").html(h.earnings.compare.percentage+b.compare_temp_2).removeClass().addClass(h.earnings.compare.classes);d(".detail-compare-first").html(h.earnings.compare.total);
d("#monthly h2").html(h.earnings.avgmonthly.earnings);d("#new-customers h2").html(h.earnings.avgpercust.current_customers);d("#new-customers span").html("This period");
d(".detail-compare-first").html(h.earnings.compare.total);d("#box-4 .bottom-text span").html(h.earnings.compare.percentage+"%");break;case"renewals":break;
