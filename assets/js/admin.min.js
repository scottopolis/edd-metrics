...
d("#yearly").html(h.earnings.avgyearly.total);d("#avgyearly-compare span").html(h.earnings.avgyearly.compare.percentage+g).removeClass().addClass(h.earnings.avgyearly.compare.classes);
...
};b.batch2response=function(f){var h=JSON.parse(f);var g=c.eddMetrics.compare_string+" "+h.dates.num_days+" "+c.eddMetrics.days;if(d("#renewals").length){d("#renewals").html(h.renewals.count);
d("#renewal-amount").html(h.renewals.earnings);d("#renewals-compare span").html(h.renewals.compare.percentage+g).removeClass().addClass(h.renewals.compare.classes);
}d("#refunds").html(h.refunds.count);d("#refund-amount").html(h.refunds.losses);d("#refunds-compare span").html(h.refunds.compare.percentage+g).removeClass().addClass(h.refunds.compare.classes);
if(d("#subscriptions").length){d("#subscriptions").html(h.subscriptions.count);d("#subscriptions-compare span").html(h.subscriptions.compare.percentage+g).removeClass().addClass(h.subscriptions.compare.classes);
...
if(d("#commissions").length){d("#commissions").html(h.commissions.count);d("#commissions-amount").html(b.currencySign+h.commissions.earnings);}};b.detailResponse=function(f){var h=JSON.parse(f);
