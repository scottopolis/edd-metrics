...
d("#discounts").text(b.currencySign+h.discounts.amount);d(".detail-compare-first").text(b.currencySign+h.earnings.compare.total);d("#box-4 .bottom-text span").text(h.earnings.compare.percentage+"%");
d("#box-5 .bottom-text span").text(h.earnings.detail.sixmoago.compare+"%");d(".detail-compare-second").text(b.currencySign+h.earnings.detail.sixmoago.total);
d(".detail-compare-third").text(b.currencySign+h.earnings.detail.twelvemoago.total);};b.detailResponse=function(f){var i=JSON.parse(f);console.log("detailResponse",i);
var h=b.getQueryVariable("metric");var g="% compared to this period";switch(h){case"revenue":d("#revenue").text(b.currencySign+i.earnings.total);d("#revenue-compare span").text(i.earnings.compare.percentage+g).removeClass().addClass(i.earnings.compare.classes);
...
d("#earnings-this-month h2").text(b.currencySign+i.earnings.detail.this_month);d("#monthly h2").text(b.currencySign+i.earnings.avgmonthly.earnings);d("#renewal-rate h2").text(i.yearly_renewal_rate.percent+"%");
d("#yearly-renewal-compare span").text("Last "+i.yearly_renewal_rate.period+" days");break;case"renewals":break;default:}b.doLineChart(i.lineChart);b.doDownloadChart(i.pieChart);
b.doGatewayChart(i.earnings.gateways);};b.getQueryVariable=function(f){var h=c.location.search.substring(1);var j=h.split("&");for(var g=0;g<j.length;g++){var k=j[g].split("=");
if(k[0]==f){return k[1];}}return(false);};b.doLineChart=function(g){if(b.lineChart){b.lineChart.destroy();}var h={labels:g.labels,datasets:[{label:"Revenue",fill:true,lineTension:0.1,backgroundColor:"rgba(0,115,170,.2)",borderColor:"#0073aa",borderCapStyle:"butt",borderDash:[],borderDashOffset:0,borderJoinStyle:"miter",pointBorderColor:"#0073aa",pointBackgroundColor:"#fff",pointBorderWidth:2,pointHoverRadius:5,pointHoverBackgroundColor:"#0073aa",pointHoverBorderColor:"rgba(220,220,220,1)",pointHoverBorderWidth:2,pointRadius:4,pointHitRadius:10,data:g.earnings,spanGaps:false},]};
