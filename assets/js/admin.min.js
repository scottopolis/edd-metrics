...
d("#sales").html(h.sales.count);d("#sales-compare span").html(h.sales.compare.percentage+g).removeClass().addClass(h.sales.compare.classes);d("#avgpercust").html(h.earnings.avgpercust.total);
d("#avgpercust-compare span").html(h.earnings.avgpercust.compare.percentage+g).removeClass().addClass(h.earnings.avgpercust.compare.classes);d("#refunds").html(h.earnings.refunds.count);
d("#refund-amount").html(h.earnings.refunds.losses);d("#refunds-compare span").html(h.earnings.refunds.compare.percentage+g).removeClass().addClass(h.earnings.refunds.compare.classes);
};b.batch2response=function(f){console.log("batch2response",f);var h=JSON.parse(f);var g=c.eddMetrics.compare_string+" "+h.dates.num_days+" "+c.eddMetrics.days;
d("#earnings-today").html(h.earnings.detail.today);if(d("#renewals").length){d("#renewals").html(h.renewals.count);d("#renewal-amount").html(h.renewals.earnings);
d("#renewals-compare span").html(h.renewals.compare.percentage+g).removeClass().addClass(h.renewals.compare.classes);}if(d("#subscriptions").length){d("#subscriptions").html(h.subscriptions.number.count);
d("#subscriptions-compare span").html(h.subscriptions.number.compare.percentage+g).removeClass().addClass(h.subscriptions.number.compare.classes);d("#recurring-revenue").html(h.subscriptions.earnings);
d("#recurring-revenue-30").html(h.subscriptions.earnings30);}d("#discounts").html(h.discounts.now.amount);d("#discounts-count").html(h.discounts.now.count);
d("#discounts-compare span").html(h.discounts.compare.percentage+g).removeClass().addClass(h.discounts.compare.classes);if(d("#commissions").length){d("#commissions").html(h.commissions.count);
d("#commissions-amount").html(h.commissions.earnings);}};b.detailResponse=function(f){var h=JSON.parse(f);var g=b.getQueryVariable("metric");switch(g){case"revenue":d("#revenue").html(h.earnings.total);
