...
d(".detail-compare-second").text("$"+h.earnings.detail.sixmoago.total);d(".detail-compare-third").text("$"+h.earnings.detail.twelvemoago.total);};b.detailResponse=function(f){console.log("detailResponse",f);
var i=JSON.parse(f);var h=b.getQueryVariable("metric");var g="% compared to this period";switch(h){case"revenue":d("#revenue").text("$"+i.earnings.total);
d("#revenue-compare span").text(i.earnings.compare.percentage+g).removeClass().addClass(i.earnings.compare.classes);d(".detail-compare-first").text("$"+i.earnings.compare.total);
d("#revenue-6mocompare span").text(i.earnings.detail.sixmoago.compare+g).removeClass().addClass(i.earnings.detail.sixmoago.classes);d(".detail-compare-second").text("$"+i.earnings.detail.sixmoago.total);
d(".detail-compare-third").text("$"+i.earnings.detail.twelvemoago.total);d("#revenue-12mocompare span").text(i.earnings.detail.twelvemoago.compare+g).removeClass().addClass(i.earnings.detail.twelvemoago.classes);
break;case"renewals":break;default:}b.doLineChart(i.lineChart);b.doPieChart(i.pieChart);};b.getQueryVariable=function(f){var h=c.location.search.substring(1);
...
var f=a.getElementById("metrics-line-chart");b.lineChart=new Chart(f,{type:"line",data:h,options:{scales:{xAxes:[{display:false,stacked:true}],}}});};b.doPieChart=function(g){if(b.pieChart){b.pieChart.destroy();
}console.log(g);var h={labels:g.labels,datasets:[{label:"Downloads",data:g.earnings,backgroundColor:["#225378","#1695A3","#1fa739","#663300","#EB7F00","#EB4B4D","#EBA945","#79EB65"],hoverBackgroundColor:["#225378","#1695A3","#1fa739","#663300","#EB7F00","#EB4B4D","#EBA945","#79EB65"]}]};
var f=a.getElementById("metrics-pie-chart");b.pieChart=new Chart(f,{type:"pie",data:h});};jQuery(a).ready(b.init);})(window,document,jQuery);