...
});}};b.dashResponse=function(f){var h=JSON.parse(f);var g=c.eddMetrics.compare_string+" "+h.dates.num_days+" "+c.eddMetrics.days;d("#revenue").text(b.currencySign+h.earnings.total);
...
};b.batch2response=function(f){var h=JSON.parse(f);var g=c.eddMetrics.compare_string+" "+h.dates.num_days+" "+c.eddMetrics.days;d("#renewals").text(h.renewals.count);
d("#renewal-amount").text(b.currencySign+h.renewals.earnings);d("#renewals-compare span").text(h.renewals.compare.percentage+g).removeClass().addClass(h.renewals.compare.classes);
...
d("#discounts").text(b.currencySign+h.discounts.now.amount);d("#discounts-count").text(h.discounts.now.count);d("#discounts-compare span").text(h.discounts.compare.percentage+g).removeClass().addClass(h.discounts.compare.classes);
};b.detailResponse=function(f){var h=JSON.parse(f);var g=b.getQueryVariable("metric");switch(g){case"revenue":d("#revenue").text(b.currencySign+h.earnings.total);
d("#revenue-compare span").text(h.earnings.compare.percentage+b.compare_temp_2).removeClass().addClass(h.earnings.compare.classes);d(".detail-compare-first").text(b.currencySign+h.earnings.compare.total);
d("#monthly h2").text(b.currencySign+h.earnings.avgmonthly.earnings);d("#new-customers h2").text(h.earnings.avgpercust.current_customers);d("#new-customers span").text("This period");
d(".detail-compare-first").text(b.currencySign+h.earnings.compare.total);d("#box-4 .bottom-text span").text(h.earnings.compare.percentage+"%");break;case"renewals":break;
