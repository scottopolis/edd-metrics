...
f.calendarSaveDates();};b.callback=function(){var j=moment(this.start_date).format("ll"),f=moment(this.end_date).format("ll");console.debug("Start Date: "+j+"\nEnd Date: "+f);
var h={action:"metrics_batch_1",start:j,end:f};var g="% over the last ";var i='<div id="circleG"><div id="circleG_1" class="circleG"></div><div id="circleG_2" class="circleG"></div><div id="circleG_3" class="circleG"></div></div>';
d(".edd-metrics-box h2").html(i);d(".edd-metrics-box .bottom-text span").html("").removeClass();if(d(this.element[0]).hasClass("metrics-detail")){d(".edd-metrics-chart-wrapper").append(i);
d.post(c.ajaxurl,h,b.detailResponse).then(function(){h.action="metrics_batch_2";d.post(c.ajaxurl,h,b.detailResponse_2);}).fail(function(){console.warn("ajax error");
});}else{d.post(c.ajaxurl,h,b.dashResponse).then(function(){h.action="metrics_batch_2";d.post(c.ajaxurl,h,b.batch2response);}).fail(function(){console.warn("ajax error");
...
};b.batch2response=function(f){var h=JSON.parse(f);console.log(h);var g="% over previous "+h.dates.num_days+" days";d("#renewals").text(h.renewals.count);
...
break;case"renewals":break;default:}b.doLineChart(i.lineChart);d(".edd-metrics-chart-wrapper #circleG").remove();};b.detailResponse_2=function(f){var i=JSON.parse(f);
console.log("detailResponse",i);var h=b.getQueryVariable("metric");var g="% compared to this period";switch(h){case"revenue":d("#revenue-6mocompare span").text(i.earnings.detail.sixmoago.compare+g).removeClass().addClass(i.earnings.detail.sixmoago.classes);
