...
function EDD_Metrics_admin_scripts( $hook ) {
    ...
    if( $hook == 'download_page_edd_metrics' ) {
	    ...
	    wp_enqueue_script( 'edd-metrics-js', EDD_Metrics_URL . 'dist/admin' . $suffix . '.js', array( 'jquery' ), EDD_Metrics_VER, true );
	    wp_enqueue_style( 'edd-metrics-css', EDD_Metrics_URL . 'dist/admin-styles' . $suffix . '.css', null, EDD_Metrics_VER );
